module punkninja::ppt {
    ...
    public entry fun add_role(caller: &signer, role_id: u8 , member: address) acquires CoinCapabilities {
        ...
        let roles = &mut borrow_global_mut<CoinCapabilities<PunkPowerToken>>(@punkninja).roles;
    ...
    /// Create new coins `PunkPowerToken` and deposit them into dst_addr's account.
    public entry fun mint(
        ...
        let caller_addr = address_of(caller);
        let capabilities = borrow_global<CoinCapabilities<PunkPowerToken>>(@punkninja);
    ...
    /// Withdraw an `amount` of coin `PunkPowerToken` from `account` and burn it.
    public entry fun burn(
        ...
        let to_burn = coin::withdraw<PunkPowerToken>(&from, amount);
